...
For instance, the library \code{Control.AllSolutions}\pindex{AllSolutions}
(available in package \code{searchtree})
defines the operation
\begin{curry}
getAllValues :: a -> IO [a]
\end{curry}
to compute all values of a given argument expression.
There is also the library \code{Control.SearchTree}\pindex{SearchTree}
(also available in package \code{searchtree})
